...
%% 2D Edc1=Edc2=0, Vt1=Vt2, z space
zdim=200;
zmin=100e-9;
zmax=500e-9;
...
z=180e-9;
H_dipol=1/(h*16*pi*e0*11.7*z^3)*(e*2*L1)*(e*2*L2)*(tensor(sigma_x,eye(4),sigma_x,eye(4))+tensor(sigma_x,eye(32))+tensor(eye(8),sigma_x,eye(4)));
...
imagesc(Edc,Vt,log(gff')/log(10));axis xy;colorbar;
title('gff (Hz)')
ylabel('Vt (Hz)')
xlabel('Edc (V/m)')
subplot(1,2,2);
imagesc(Edc,Vt,leak');axis xy;colorbar;
title('leakage')
ylabel('Vt (Hz)')
xlabel('Edc (V/m)')
...
for ll = 1:Vtdim
    ...
    gorb=Vt(ll)*pi;
        ...
        save 2QFid_noise_adiabatic_Vt_02.mat
        toc;
        ll
%         (ll-1)/Vtdim+(mm-1)/Vtdim/EdcFdim+1/Vtdim/EdcFdim
end
...
figure(67381);
subplot(1,3,1);
plot((Vt-sqrt(((28e9+17.2e6-28e6)*B0)^2 + A^2/4))/1e6,ttgate*1e9);
ylabel('Gate time (ns)')
xlabel('V_t - \epsilon_{ff} (MHz)')
subplot(1,3,2);
plot((Vt-sqrt(((28e9+17.2e6-28e6)*B0)^2 + A^2/4))/1e6,log(abs(1-Fid16))/log(10));
ylabel('log_{10}(Leakage error)')
xlabel('V_t - \epsilon_{ff} (MHz)')
subplot(1,3,3);
plot((Vt-sqrt(((28e9+17.2e6-28e6)*B0)^2 + A^2/4))/1e6,log(1-FidNoiseAv)/log(10));
ylabel('log_{10}(Total error)')
xlabel('V_t - \epsilon_{ff} (MHz)')
...
%                 Fid16(mm,ll,kkkk,:,(Edcnoisedim+1)/2,(Edcnoisedim+1)/2)
            ...
            save 2QFid_noise_adiabatic_RMS_K_07.mat
        end
%         (ll-1)/Vtdim+(mm-1)/Vtdim/EdcFdim+1/Vtdim/EdcFdim
        toc;
    end
