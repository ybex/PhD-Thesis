...
A=117e6;
B0=0.2*2;
Vt=11.46583e9;%sqrt(((28e9+17.2e6-28e6)*B0)^2 + A^2/4)+600e6;%-3e8;%6e9; %double-dot tunel rate
Edc=000;%-110;%-330;
...
xlim([-.3 34.8])
ylim([-10 110])
set(gca,'XTick',[0 10 20 30],'YTick',[0 100],'Position', [0.15 0.65 0.8 0.3],'FontSize',50,'XTickLabel','','ticklength',[0.02 0.1])
...
xlim([-.3 34.8])
ylim([-1.1 1.1])
set(gca,'XTick',[0 10 20 30],'YTick',[-1 0 1],'Position', [0.15 0.15 0.8 0.45],'FontSize',50,'ticklength',[0.02 0.1])
...
for ll = 1:Vtdim
    ...
    for mm = 1:EdcDim
        ...
        for t=1:(kk-1)
            Operator=H_expm(:,:,t)*Operator;
            ge=ge+Operator'*charup*Operator;
        end
        ...
        for ii=1:4
            RelErr(ll,mm)=RelErr(ll,mm)+(Basis(:,ii)'*ge*Basis(:,ii))/4;
        end
        RelErr(ll,mm)=RelErr(ll,mm)*2.37e-24*eo*Vt(ll)^2;
        RelErr(ll,mm)=(1-exp(-tpi2(ll,mm)*RelErr(ll,mm)))/2
...
title('log_{10}(Leakage error)')
xlabel('Edc final (V/m)')
ylabel('V_t - \epsilon_{ff} (MHz)')
subplot(2,2,3);
imagesc(-Edc,(Vt-sqrt(((28e9+17.2e6-28e6)*B0)^2 + A^2/4))/1e6,log(1-Fid4av)/log(10));axis xy;colorbar;
title('log_{10}(Noise error)')
xlabel('Edc final (V/m)')
ylabel('V_t - \epsilon_{ff} (MHz)')
subplot(2,2,4);
imagesc(-Edc,(Vt-sqrt(((28e9+17.2e6-28e6)*B0)^2 + A^2/4))/1e6,log(abs(RelErr))/log(10));axis xy;colorbar;
title('log_{10}(Relaxation error)')
xlabel('Edc final (V/m)')
ylabel('V_t - \epsilon_{ff} (MHz)')
...
figure(673811);
subplot(2,2,1);
plot((Vt-sqrt(((28e9+17.2e6-28e6)*B0)^2 + A^2/4))/1e6,tpi2(:,19)*1e9);
ylabel('Gate time (ns)')
xlabel('V_t - \epsilon_{ff} (MHz)')
subplot(2,2,2);
plot((Vt-sqrt(((28e9+17.2e6-28e6)*B0)^2 + A^2/4))/1e6,log(abs(1-Fid4(:,19)))/log(10));
ylabel('log_{10}(Leakage error)')
xlabel('V_t - \epsilon_{ff} (MHz)')
subplot(2,2,3);
plot((Vt-sqrt(((28e9+17.2e6-28e6)*B0)^2 + A^2/4))/1e6,log(1-Fid4av(:,19))/log(10));
ylabel('log_{10}(Noise error)')
xlabel('V_t - \epsilon_{ff} (MHz)')
subplot(2,2,4);
plot((Vt-sqrt(((28e9+17.2e6-28e6)*B0)^2 + A^2/4))/1e6,log(RelErr(:,19))/log(10));
...
for ll = 1:Vtdim
    ...
    for ii=1:4
        RelErr(ll)=RelErr(ll)+(Basis(:,ii)'*ge*Basis(:,ii))/4;
    end
    RelErr(ll)=RelErr(ll)*2.37e-24*eo*Vt(ll)^2;
    RelErr(ll)=(1-exp(-tpi2(ll)*RelErr(ll)))/2
    ...
    toc;
    save 1Q-fid_adiabatic_Vt_05.mat
end
profile viewer
...
xlabel('V_t - \epsilon_{ff} (MHz)')
subplot(2,2,2);
plot((Vt-sqrt(((28e9+17.2e6-28e6)*B0)^2 + A^2/4))/1e6,log10(Power));
ylabel('log_{10}(Power) (W)')
xlabel('V_t - \epsilon_{ff} (MHz)')
subplot(2,3,4);
...
xlabel('V_t - \epsilon_{ff} (MHz)')
subplot(2,3,6);
plot((Vt-sqrt(((28e9+17.2e6-28e6)*B0)^2 + A^2/4))/1e6,log(RelErr)/log(10));
ylabel('log_{10}(Relaxation error)')
xlabel('V_t - \epsilon_{ff} (MHz)')
...
for ll = 1:Vtdim
    ...
    for kkkk=1:Kdim
        for eed=1:Edcdim
            ...
            toc;
            ll,kkkk,eed
            save 1Q-fid_adiabatic_5D_09.mat
        end
    end
end
profile viewer
