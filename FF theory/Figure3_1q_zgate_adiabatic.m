...
% this determines the time steps dt for the following simulations according
% to adiabaticity
...
dDorbdEz=e*2*L*pi/h; %Delta_c derivative
gorb=Vt*pi; %Omega_c

%charge qubit adiabaticity
for kk=1:Edcdim
    ...
    Dorb=(e*Edc(kk)*2*L/h)*pi; %Delta_c
...
c=colormap(parula)
Edc_p=Edc*1e-3;
H2=semilogy(Edc_p,dt./dEdcMeas*1e9, Edc_p,dtSO./dEdcMeas*1e9,Edc_p,dtOrb./dEdcMeas*1e9);
ylabel('dt/dE_z (ns)');
xlabel('(E_z-E_0) (kV/m)');
xlim([-30 -0])
set(H2(1),'Marker','none','LineStyle','-','LineWidth',2,'Color',c(1, :));
set(H2(2),'Marker','none','LineStyle','--','LineWidth',2,'Color',c(35,:));
set(H2(3),'Marker','none','LineStyle','--','LineWidth',2,'Color',c(55,:));
legend( 'dt', 'dtSO','dtOrb', 'Location', 'southeast')
set(gca,'FontSize',18, 'LineWidth', 3);
%legend('Nemo3D similation', 'theory')
set(gca,'color','none')
print('adiabatic.pdf','-r0','-dpdf')
...
aa=0;
bb=0;
%find realEigV with max overlap with Up and Down unperturbed states (get
%alpha and beta)
for i=1:8
...
UP=sign(UP*H_eVec(:,aa))*H_eVec(:,aa); %rounding to basis? %1
DOWN=sign(DOWN*H_eVec(:,bb))*H_eVec(:,bb); %3

OperStep=zeros(8,8,Edcdim);
Operator=eye(8);
nuE=zeros(1,Edcdim);

down=tensor([1 0]'*[1 0],eye(2),[0 1]'*[0 1]); %ff
up=tensor([0 1]'*[0 1],eye(2),[1 0]'*[1 0]);
int=tensor(eye(2),[1 -1]'*[1 -1]./2,eye(2)); %interface state
don=tensor(eye(2),[1 1]'*[1 1]./2,eye(2)); %donor state
charup=tensor(eye(2),Orb_eVec(:,2)*Orb_eVec(:,2)',eye(2)); %excited charge state
%chardown=tensor(eye(2),Orb_eVec(:,1)*Orb_eVec(:,1)',eye(2));

Psit = zeros(8,2*Edcdim+gatedim+1);
Psit(:,1)=(1*UP+1*DOWN)/sqrt(2); %initial state

FFqubit=zeros(1,2*Edcdim+gatedim+1); %sigma z ff
FFx=zeros(1,2*Edcdim+gatedim+1); %sigma x ff
position=zeros(1,2*Edcdim+gatedim+1); %sigma z charge
ge=zeros(1,2*Edcdim+gatedim+1); %charge excitation
...
%moving the electron to the operating point
for t=1:Edcdim
...
%moving the electron back to the idle point
