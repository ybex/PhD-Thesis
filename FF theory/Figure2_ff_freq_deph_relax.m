...
B0=0.2*2;
Vt=sqrt(((28e9+17.2e6-28e6)*B0)^2 + A^2/4)+243e6; %this is the ff freq + a small amount (detuning+dorb)
...
orb_e=tensor([1 0],[1 0],[0 1]); %|ueD>
flip_e=tensor([0 1],[0 1],[1 0]); %|dgU>
orbflip_g=tensor([1 0],[0 1],[0 1]); %|ugD>
...
for ii=1:Edcdim
    ...
    Vtt(ii,1)=abs(H_eVal(aa,aa)-H_eVal(cc,cc)); %g->e orbital transition
    fidVtt(ii,1)=fid1*fid3;
    ggB0(ii,1)=abs(H_eVal(bb,bb)-H_eVal(cc,cc)); %flip flop transition with orbital state g
    fidggB0(ii,1)=fid2*fid3;
    
    %fahd
    Vtp = sqrt(Vt^2+(e*2*L*Edc(ii)/h)^2); %charge qubit energy
    gp = 1+(1/2+((e*2*L*Edc(ii)/h)/Vtp)/2)*dg; %g tensor change
    Ap = A/2*(1-((e*2*L*Edc(ii)/h)/Vtp)); %hyperfine change
    Ap2(ii)=Ap;
    theta=((e*2*L*Edc(ii)/h)-sqrt((e*2*L*Edc(ii)/h)^2+Vt^2))/Vt
    test2(ii)=(theta/sqrt(1+theta^2))^2*A;
    
    E=(e*2*L*Edc(ii)/h);
    test3(ii)=(2*E^2+Vt^2-2*E*Vtp)/(2*E^2+2*Vt^2-2*E*Vtp)*A;
    test4(ii)=(E-Vtp)^2/(Vt^2+(E-Vtp)^2)*A;
...
%%
figure(2)
plot(Edc/1000,Ap2/1e9,'Color',[.5 .5 .5],'LineWidth',1.5);
%plot(Edc/1000,test2/1e9,'Color','b','LineStyle','-');
hold on
plot(Edc/1000,test3/1e9,'Color','g','LineStyle','--');
hold on
