...
while phase1<=pi/4
    ...
    dgE=(K^-1)*(DE^2+gE^2)^1.5*dt/DE; %this is the adiabaticity formula to adiabically increase Eac at fixed dt
    
    % find phase as a function of Eac and time
    H_expm_floquet=eye(8);
    
    %time evolution of H with floquet theory
    for tt=1:Tcoarse
        H=H_0+cos(2*pi*tt/Tcoarse)*tensor(identity,e*Eac(kk)*L/h*sigma_x,identity);  
        H_expm_floquet=expm(-1i*2*pi*H/(nuE*Tcoarse))*H_expm_floquet; %mutiply for Tcoarse time evolution
    end
    H_expm(:,:,kk)=H_expm_floquet^(dt*nuE); %finialized evolved H
    %------
    
    if kk~=1
        tmax=(eo-nuE)/(e*Eac(kk)*2*L*A/h/16*Vt^2/eo^2)*40; %deltaE/(gso*gE)*40?
        tdim1=tmax*nuE;
...
for ll=1:Edcnoisedim
    ...
    H_expm_floquet=eye(8);
    for tt=1:Tcoarse
        H=H_0+cos(2*pi*tt/Tcoarse)*tensor(identity,e*Eac(kk-1)*L/h*sigma_x,identity); %H
        H_expm_floquet=expm(-1i*2*pi*H/(nuE*Tcoarse))*H_expm_floquet; % time evolution until Tcourse of H
    end
    H_expm(:,:,kk-1)=H_expm_floquet^(dtp*nuE);
